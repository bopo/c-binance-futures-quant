...
def shortsStopLossByPrice(symbol, stopLossPrice, quantity):
    ...
    try:
        for i in range(stopTime):
            ...
            result = REQUEST_CLIENT.post_auto_order_with_price(price=tradePrice, newClientOrderId=newClientOrderId, reduceOnly=True, symbol=symbol, quantity=stopQuantity,
                                                               side=OrderSide.BUY, ordertype=OrderType.STOP, stopPrice=stopLossPrice, positionSide='BOTH',
...
def longsStopLossByPrice(symbol, stopLossPrice, quantity):
    ...
    try:
        for i in range(stopTime):
            ...
            stopLossPrice = decimal.Decimal(PRICE_DECIMAL_OBJ[symbol] % (stopLossPrice))
            result = REQUEST_CLIENT.post_auto_order_with_price(price=tradePrice, newClientOrderId=newClientOrderId, reduceOnly=True, symbol=symbol, quantity=stopQuantity,
                                                               side=OrderSide.SELL, ordertype=OrderType.STOP, stopPrice=stopLossPrice, positionSide='BOTH',
                                                               timeInForce=TimeInForce.GTC)
            result = json.loads(result)
            if 'code' in result and str(result['code']) == '4183':
