            ...
            for a in range(len(klineArr)):
                ...
                for b in range(len(klineArr[a])):
                    ...
                    if localMin == klineMin:
                        ONE_MIN_KLINE_OBJ_ARR[a]['klineArr'][len(ONE_MIN_KLINE_OBJ_ARR[a]['klineArr']) - 1] = [klineArr[a][b][1], klineArr[a][b][4], klineArr[a][b][2],
            ...
            for a in range(len(priceArr)):
                ...
                if localMin == serverMin:
                    ...
                    if localMin - 1 != serverMin and (localMin != 1 and localMin != 0):
                        _thread.start_new_thread(FUNCTION_CLIENT.send_lark_msg_limit_one_min,
                                                 ('localMin:' + str(localMin) + ',serverMin:' + str(serverMin) + ',symbol:' + str(TRADE_SYMBOL_ARR[a]['symbol']),))
                        LOCAL_ONE_MIN_KLINE_OBJ_ARR = []
            ...
            for a in range(len(ONE_MIN_KLINE_OBJ_ARR)):
                for b in range(len(LOCAL_ONE_MIN_KLINE_OBJ_ARR[a])):
                    ...
                    if klineMin == localMin:
                        ...
                        ONE_MIN_KLINE_OBJ_ARR[a]['klineArr'].append(
                            [LOCAL_ONE_MIN_KLINE_OBJ_ARR[a][b][0], LOCAL_ONE_MIN_KLINE_OBJ_ARR[a][b][1], LOCAL_ONE_MIN_KLINE_OBJ_ARR[a][b][2], LOCAL_ONE_MIN_KLINE_OBJ_ARR[a][b][3],
                             localMin])
                        del ONE_MIN_KLINE_OBJ_ARR[a]['klineArr'][0]
                        ONE_MIN_KLINE_OBJ_ARR[a]['dataError'] = False
...
def makerShortsOrder(shortsPrice, shortsOnceTradeCoinQuantity, symbol):
    ...
    try:
        result = REQUEST_CLIENT.post_order(newClientOrderId=newClientOrderId, reduceOnly=False, symbol=symbol, quantity=coinQuantity, side=OrderSide.SELL,
                                           ordertype=OrderType.LIMIT, price=shortsPrice, positionSide='BOTH', timeInForce=TimeInForce.GTX)
...
def makerCloseLongsOrder(shortsPrice, shortsOnceTradeCoinQuantity, symbol):
    ...
    result = {}
    try:
        result = REQUEST_CLIENT.post_order(newClientOrderId=newClientOrderId, reduceOnly=True, symbol=symbol, quantity=coinQuantity, side=OrderSide.SELL, ordertype=OrderType.LIMIT,
                                           price=shortsPrice, positionSide='BOTH', timeInForce=TimeInForce.GTX)
        result = json.loads(result)
        if 'code' in result and result['code'] != -5022 and result['code'] != -1001 and result['code'] != -2022:
...
def makerLongsOrder(longsPrice, longsOnceTradeCoinQuantity, symbol):
    ...
    try:
        result = REQUEST_CLIENT.post_order(newClientOrderId=newClientOrderId, reduceOnly=False, symbol=symbol, quantity=coinQuantity, side=OrderSide.BUY, ordertype=OrderType.LIMIT,
                                           price=longsPrice, positionSide='BOTH', timeInForce=TimeInForce.GTX)
        result = json.loads(result)
...
def makerCloseShortsOrder(longsPrice, longsOnceTradeCoinQuantity, symbol):
    ...
    result = {}
    try:
        result = REQUEST_CLIENT.post_order(newClientOrderId=newClientOrderId, reduceOnly=True, symbol=symbol, quantity=coinQuantity, side=OrderSide.BUY, ordertype=OrderType.LIMIT,
                                           price=longsPrice, positionSide='BOTH', timeInForce=TimeInForce.GTX)
        result = json.loads(result)
        if 'code' in result and result['code'] != -5022 and result['code'] != -1001 and result['code'] != -2022:
...
def newOpenOrders():
    ...
    for oneMinKlineObjArrIndex in range(len(ONE_MIN_KLINE_OBJ_ARR)):
        ...
        oneMinRate = FUNCTION_CLIENT.get_percent_num(float(oneMinArr[len(oneMinArr) - 1][1]) - float(oneMinArr[len(oneMinArr) - 1 - 1][1]),
