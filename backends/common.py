...
class FunctionClient(object):
    def __init__(self, **params):
        ...
        if 'connectMysql' in params and params['connectMysql']:
            self.mysqlConnect = mysql.connector.connect(**settings.MYSQL_CONFIG)

        self.mysqlPoolConnect = {}
        if 'connectMysqlPool' in params and params['connectMysqlPool']:
            self.mysqlPoolConnect = MySQLConnectionPool(pool_name='mypool', pool_size=30, **settings.MYSQL_CONFIG)
    ...
    def open_take_binance_orders_by_web_server(self, symbol, direction, key, secret, price, openTime, positionValue, volMultiple):
        try:
            url = 'http://' + settings.TRADE_WEB_ADDRESS + ':8888/take_open'

            postDataObj = {'privateIP': self.privateIP, 'volMultiple': volMultiple, 'symbol': symbol, 'direction': direction, 'key': key, 'secret': secret, 'price': price,
                           'openTime': openTime, 'positionValue': positionValue}
            response = requests.request('POST', url, timeout=(3, 3), data=postDataObj)
        except Exception as e:
            print(e)
    ...
    def end_open_by_web_server(self, symbol):
        try:
            url = 'http://' + settings.TRADE_WEB_ADDRESS + ':8888/end_open'
    ...
    def begin_trade_record(self, volMultiple, standardRate, symbol, klineArr, nowOpenRate, machineNumber, direction, myTradeType, longsConditionA, shortsConditionA,
                           shortsConditionB, btcNowOpenRate, ethNowOpenRate, clientBeginPrice, clientEndPrice):
        try:
