...
def main():
    ...
    for a in range(len(symbolsArr)):
        if symbolsArr[a]['status'] == 'TRADING' and symbolsArr[a]['deliveryDate'] == 4133404800000 \
            and symbolsArr[a]['underlyingType'] != 'INDEX' and symbolsArr[a]['quoteAsset'] == 'USDT':
            ...
            FUNCTION_CLIENT.mysql_commit(sql, [thisSymbol, thisBaseAsset, thisQuote, 'yes', FUNCTION_CLIENT.turn_ts_to_time(int(symbolsArr[a]['onboardDate'] / 1000)), 0, 0,
                                               int(symbolsArr[a]['onboardDate'] / 1000), json.dumps([])])
