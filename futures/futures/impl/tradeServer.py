...
def register():
    ...
    do_commit(sql, [ip, account, password, name, turnTsToTime(int(time.time())), json.dumps([]), json.dumps(json.loads(newHotKeyConfigObj)), json.dumps({}), json.dumps({}),
                    accessToken])
    resp = json.dumps({'s': 'ok'})
    response.set_header('Access-Control-Allow-Origin', '*')
    return resp
...
def login():
    ...
    if len(userData) == 0:
        ...
        resp = json.dumps({'s': 'ok', 'account': account, 'password': userData[0][0], 'usdtAssets': userData[0][1], 'binanceApiArr': json.loads(userData[0][2]),
                           "hotKeyConfigObj": json.loads(userData[0][3]), "stateConfigObj": json.loads(userData[0][4]), "serverInfoObj": json.loads(userData[0][5]),
                           "name": userData[0][6], "accessToken": userData[0][7]})
        response.set_header('Access-Control-Allow-Origin', '*')
        return resp
...
def trade():
    ...
    if tradeType == "marketOpenLongs":
        ...
        request_client = RequestClient(api_key=apiKey, secret_key=API_OBJ[apiKey])
        result = request_client.post_order(newClientOrderId=newClientOrderId, reduceOnly=True, symbol=symbol, quantity=marketMaxSize, side=OrderSide.SELL,
                                           ordertype=OrderType.LIMIT, price=thisPrice, positionSide="BOTH", timeInForce=TimeInForce.GTC)
        result = json.loads(result)
        coinQuantity = float(decimal.Decimal(AMOUNT_DECIMAL_OBJ[symbol] % (money / price)))
