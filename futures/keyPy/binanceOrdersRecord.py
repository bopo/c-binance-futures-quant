    ...
    if "code" in result:
        ...
        for i in range(len(result)):
            ...
            if not noInsert:
                ...
                FUNCTION_CLIENT.mysql_commit(sql,
                                             [avgPrice, clientOrderId, cumQuote, executedQty, orderId, origQty, origType, price, reduceOnly, side, positionSide, status, stopPrice,
                                              closePosition, symbol, timeInForce, orderType, updateTime, workingType, priceProtect, binanceTs, myTs])
            if update:
                ...
                FUNCTION_CLIENT.mysql_commit(sql,
                                             [avgPrice, clientOrderId, cumQuote, executedQty, orderId, origQty, origType, price, reduceOnly, side, positionSide, status, stopPrice,
                                              closePosition, symbol, timeInForce, orderType, updateTime, workingType, priceProtect, binanceTs, myTs, DBID])
...
def updateTrade(symbol):
    ...
    if len(data) > 0:
        ...
        if "code" in result:
            ...
            sql = "update " + ORDERS_TABLE_NAME + " set `avgPrice`=%s,`clientOrderId`=%s,`cumQuote`=%s,`executedQty`=%s,`orderId`=%s,`origQty`=%s,`origType`=%s,`price`=%s,`reduceOnly`=%s,`side`=%s,`positionSide`=%s,`status`=%s,`stopPrice`=%s,`closePosition`=%s,`symbol`=%s,`timeInForce`=%s,`orderType`=%s,`updateTime`=%s,`workingType`=%s,`priceProtect`=%s,`binanceTs`=%s,`myTs`=%s  where id=%s"
            FUNCTION_CLIENT.mysql_commit(sql, [avgPrice, clientOrderId, cumQuote, executedQty, orderId, origQty, origType, price, reduceOnly, side, positionSide, status, stopPrice,
                                               closePosition, symbol, timeInForce, orderType, updateTime, workingType, priceProtect, binanceTs, myTs, data[0][1]])
