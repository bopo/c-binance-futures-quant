        ...
        if "code" in result:
            ...
            for i in range(len(result)):
                ...
                if incomeType == "REALIZED_PNL":
                    ...
                    for b in range(scanCount):
                        if (str(int(incomeData[b][0])) == str(int(binance_ts))) and (format(float(incomeData[b][1]), '.8f') == format(float(income), '.8f')) and (
                                str(incomeData[b][2]) == str(trade_id)):
                            isExit = True
                    if not isExit:
            ...
            for i in range(len(result)):
                ...
                for b in range(len(lastBinanceTsData)):
                    if (str(int(lastBinanceTsData[b][0])) == str(int(binance_ts))) and (str(lastBinanceTsData[b][1]) == str(incomeType)) and (
                ...
                if not isExit and result[i]['time'] > 1688256000000:
                    insertSQLStr = "('" + str(incomeType) + "','" + str(income) + "','" + str(
                        asset) + "','" + info + "','" + trade_id + "','" + binance_ts + "','" + my_ts + "','" + symbol + "','" + symbol + "','" + symbol + "','" + str(
                        bnbPrice) + "')"
